...
class GlobalViewModel(
    ...
    fun updateImage(id: String, file: File, name: String): LiveData<Resource<responseGeneral>> =
        liveData {
            emit(Resource.Loading())
            try {
    ...
    fun createPersonal(
        ...
        file: File,
        name: String
    ): LiveData<Resource<responseGeneral>> = liveData {
        emit(Resource.Loading())
        try {
    ...
    fun changePasswordPersonal(id: String, password: String): LiveData<Resource<responseGeneral>> =
        liveData {
            emit(Resource.Loading())
            try {
                val respuesta = repo.changePasswordPersonal(id, password)
                emit(Resource.Success(respuesta))
            } catch (e: Exception) {
                emit(Resource.Failure(e))
            }
        }
    ...
    fun putComerciante(id: String, usuario: Comerciantes): LiveData<Resource<responseGeneral>> =
        liveData {
            emit(Resource.Loading())
            try {
    ...
    fun uploadExcel(file: File, name: String, tipe: String): LiveData<Resource<responseGeneral>> =
        liveData {
            emit(Resource.Loading())
            try {
                val nameFile = RequestBody.create(MediaType.parse("text/plain"), name)
                val archivo = guardarExcelArchivo(name, file, tipe)
